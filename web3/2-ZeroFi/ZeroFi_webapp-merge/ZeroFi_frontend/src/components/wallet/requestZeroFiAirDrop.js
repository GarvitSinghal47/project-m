...
const ZeroFiProgramId = new PublicKey(
  "61Yze1wz1D1adaAmuqKnrCBTHhdmW9BmC64Ejv5XK9Hc"
);
var ZeroFiCoinTestTokenMintPubkey = new PublicKey(
...
const RequestZeroFiCoinAirdrop = () => {
  const classes = useStyles();
  const { connection } = useConnection();
  const { publicKey, sendTransaction } = useWallet();
  const notify = useNotify();
  // calling this function will airdrop 250 ZeroFiCoin Test tokens in user wallet at devnet network
  const onClick = useCallback(async () => {
    ...
    try {
      ...
      await findAssociatedTokenAddress(
        publicKey,
        ZeroFiCoinTestTokenMintPubkey
      ).then(async function (ZeroFiAssociatedAccPubkey) {
        console.log(ZeroFiAssociatedAccPubkey.toString());
        ...
        const createFantAssociatedAccountIx =
          Token.createAssociatedTokenAccountInstruction(
            ...
            TOKEN_PROGRAM_ID,
            ZeroFiCoinTestTokenMintPubkey,
            ZeroFiAssociatedAccPubkey,
            publicKey,
            publicKey
          );

        const ZeroFiAirdropUserStorageAccountPk =
        ...
        const createFantAirdropUserStorageDataAccountIx =
          SystemProgram.createAccountWithSeed({
            ...
            newAccountPubkey: ZeroFiAirdropUserStorageAccountPk,
            programId: ZeroFiProgramId,
            seed: "DassiFinanceAirdrop",
            space: 8,
          });

        await connection
          .getAccountInfo(ZeroFiAirdropUserStorageAccountPk)
          .then((account_info) => {
        ...
        await connection
          .getAccountInfo(ZeroFiAssociatedAccPubkey)
          .then((account_info) => {
        ...
        const getAirdropTokensIx = new TransactionInstruction({
          ...
          keys: [
            ...
            {
              pubkey: ZeroFiAirdropUserStorageAccountPk,
              isSigner: false,
              isWritable: true,
            },
            {
              pubkey: ZeroFiAssociatedAccPubkey,
              isSigner: false,
              isWritable: true,
            },
            {
  ...
  return (
    <Button className={classes.root} onClick={onClick} disabled={!publicKey}>
      Airdrop ZeroFiCoin (devnet)
    </Button>
  );
};

export default RequestZeroFiCoinAirdrop;
