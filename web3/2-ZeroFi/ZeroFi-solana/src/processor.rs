...
use crate::{
    error::ZeroFiError, instruction::ZeroFiInstruction, state, state::AccTypes,
...
const ZeroFi_COIN_DECIMALS: u64 = 1000_000_000;
...
impl Processor {
    pub fn process(
        ...
        msg!("in processor");
        let instruction = ZeroFiInstruction::unpack(instruction_data)?;
        msg!("out instruction");
        match instruction {
            ZeroFiInstruction::LendToBorrower {
                amount_to_lend_input,
                lender_id_input,
            } => {
                msg!("ZeroFiInstruction::LendToBorrower");
            ...
            ZeroFiInstruction::WithdrawLenderFreeWalletFunds { lender_id_input } => {
                msg!("ZeroFiInstruction::WithdrawLenderFreeWalletFunds");
            ...
            ZeroFiInstruction::WithdrawCollectedLoanFunds {} => {
                msg!("ZeroFiInstruction::WithdrawCollectedLoanFunds");
                Self::process_withdraw_collected_loan_funds(accounts, program_id)
            }

            ZeroFiInstruction::TransferZeroFiVaultAccountOwnership {} => {
                msg!("ZeroFiInstruction::TransferZeroFiVaultAccountOwnership");
                Self::process_transfer_ZeroFi_vault_account_ownership(accounts, program_id)
            }

            ZeroFiInstruction::InitializeLendersStorageAccount {} => {
                msg!("ZeroFiInstruction::InitializeLendersStorageAccount");
                Self::process_initialize_lenders_storage_account(accounts)
            }
            ZeroFiInstruction::InitializeBorrowerAccount {} => {
                msg!("ZeroFiInstruction::InitializeBorrowerAccount");
                Self::process_initialize_borrower_storage_account(accounts, program_id)
            }
            ZeroFiInstruction::InitializeGuarantorAccount {} => {
                msg!("ZeroFiInstruction::InitializeGuarantorAccount");
                Self::process_initialize_guarantor_storage_account(accounts, program_id)
            }

            ZeroFiInstruction::PayEMIforLoan {
                emi_amount_to_pay_input,
            } => {
                msg!("ZeroFiInstruction::PayEMIforLoan");
                Self::process_pay_emi(accounts, emi_amount_to_pay_input, program_id)
            }

            ZeroFiInstruction::InitializeLoanInfoAccount {
                ...
                msg!("ZeroFiInstruction::InitializeLoanInfoAccount");
            ...
            ZeroFiInstruction::AirdropUsersWithZeroFiTestCoins {} => {
                msg!("ZeroFiInstruction::AirdropUsersWithZeroFiTestCoins");
                Self::process_airdrop_users_with_ZeroFi_test_coins(accounts, program_id)
            }

            ZeroFiInstruction::TransferAirdropVaultAccountOwnership {} => {
                msg!("ZeroFiInstruction::TransferAirdropVaultAccountOwnership");
                Self::process_transfer_airdrop_vault_account_ownership(accounts, program_id)
            }

            ZeroFiInstruction::ReturnFundsToLenders { num_accounts_input } => {
                msg!("ZeroFiInstruction::ReturnFundsToLenders");
                Self::process_return_funds_to_lenders(accounts, num_accounts_input, program_id)
            }

            ZeroFiInstruction::CloseLoanInfoAccount {} => {
                msg!("ZeroFiInstruction::CloseLoanInfoAccount");
    ...
    fn process_lend_to_borrower(
        ...
        let lender_ZeroFi_coin_account_to_debit = next_account_info(account_info_iter)?;

        // ZeroFi_coin_vault_account is program controlled vault account and can be only controlled by our deployed program for debit funds or any kind of operation
        let ZeroFi_coin_vault_account = next_account_info(account_info_iter)?;
        ...
        if token_program.key != &spl_token::id() {
            return Err(ZeroFiError::InvalidTokenProgram.into());
        }

        let ZeroFi_coin_vault_account_data_before =
            TokenAccount::unpack(&ZeroFi_coin_vault_account.data.borrow())?;
        let ZeroFi_coin_vault_balance_before = ZeroFi_coin_vault_account_data_before.amount;

        let (pda_ZeroFi_vault, _bump_seed) =
            Pubkey::find_program_address(&[b"ZeroFiFinance"], program_id);

        if ZeroFi_coin_vault_account_data_before.owner != pda_ZeroFi_vault {
            return Err(ZeroFiError::ZeroFiVaultAccountDoesNotMatched.into());
        }

        let transfer_lending_amount_to_vault_ix = spl_token::instruction::transfer(
            token_program.key,
            lender_ZeroFi_coin_account_to_debit.key,
            ZeroFi_coin_vault_account.key,
        ...
        msg!(
            "amount of ZeroFi coin tokens to transfer {}, lender debit key {}",
            (amount_to_lend_input as f64 / ZeroFi_COIN_DECIMALS as f64),
            lender_ZeroFi_coin_account_to_debit.key.to_string()
        );

        invoke(
            &transfer_lending_amount_to_vault_ix,
            &[
                lender_ZeroFi_coin_account_to_debit.clone(),
                ZeroFi_coin_vault_account.clone(),
                lender_main_account.clone(),
                token_program.clone(),
            ],
        )?;

        let ZeroFi_coin_vault_account_data_after =
            TokenAccount::unpack(&ZeroFi_coin_vault_account.data.borrow())?;
        let ZeroFi_coin_vault_balance_after = ZeroFi_coin_vault_account_data_after.amount;
        msg!(
            "ZeroFiCoin vault balance after: {}",
            ZeroFi_coin_vault_balance_after
        );
        let vault_balance_increased = ZeroFi_coin_vault_balance_after
            .checked_sub(ZeroFi_coin_vault_balance_before)
            .unwrap();
    
        // Minimum ZeroFiCoin to lend = 10
        if vault_balance_increased < MIN_LENDING_AMOUNT {
            return Err(ZeroFiError::ExpectedAmountMismatch.into());
        }
       
        let loan_info_storage_account = next_account_info(account_info_iter)?;
       
        if loan_info_storage_account.owner != program_id {
            return Err(ZeroFiError::WrongAccountPassed.into());
        }
        ...
        if lenders_storage_data_byte_array[0] != AccTypes::LendersAcc as u8 {
            return Err(ZeroFiError::ExpectedAccountTypeMismatched.into());
        }
      
        if lenders_storage_data_byte_array[1] != 1u8 {
            return Err(ZeroFiError::ExpectedLendersAccNumNotMatched.into());
        }   
     
        if lender_id_input > 49_999u32 {
            return Err(ZeroFiError::InvalidLenderIdInput.into());
        }
            ...
            && loan_info_header_data.total_amount_lended < loan_info_header_data.total_loan_amount
        {
            ...
            return Err(ZeroFiError::FundraisingPeriodExpired.into());
        }
     
        if loan_info_header_data.total_amount_lended >= loan_info_header_data.total_loan_amount {
            return Err(ZeroFiError::BorrowerAlreadyFunded.into());
        }
    ...
    fn process_pay_emi(
        ...
        let borrower_ZeroFi_coin_account_to_debit = next_account_info(account_info_iter)?;
        ...
        */
        ...
        let transfer_emi_amount_to_vault_ix = spl_token::instruction::transfer(
            token_program.key,
            borrower_ZeroFi_coin_account_to_debit.key,
            ZeroFi_coin_vault_account.key,
            borrower_main_account.key,
            &[],
            emi_amount_to_pay_input,
        ...
        msg!(
            "amount of ZeroFi coin tokens to transfer {}, borrower debit key {}",
            (emi_amount_to_pay_input as f64 / ZeroFi_COIN_DECIMALS as f64),
            borrower_ZeroFi_coin_account_to_debit.key.to_string()
        );

        invoke(
            &transfer_emi_amount_to_vault_ix,
            &[
                borrower_ZeroFi_coin_account_to_debit.clone(),
                ZeroFi_coin_vault_account.clone(),
        ...
        {
            return Err(ZeroFiError::LoanAlreadyPaid.into());
        }
    ...
    fn initialize_loan_info_account(
        ...
        */
        ...
        if borrower_data.is_active_loan != 0 {
            return Err(ZeroFiError::BorrowerAlreadyHaveActiveLoan.into());
        }
    ...
    fn process_withdraw_lender_free_wallet_funds(
        ...
        let lender_ZeroFi_coin_account_to_credit = next_account_info(account_info_iter)?;
        ...
        */
        ...
        let (pda, bump_seed) = Pubkey::find_program_address(&[b"ZeroFiFinance"], program_id);
        if pda != *pda_account.key {
            return Err(ZeroFiError::PdaAccountDoesNotMatched.into());
        }

        let transfer_withdraw_amount_to_lender_ix = spl_token::instruction::transfer(
            token_program.key,
            ZeroFi_coin_vault_account.key,
            lender_ZeroFi_coin_account_to_credit.key,
            &pda,
            &[&pda],
            withdraw_amount,
        )?;
        ...
        msg!(
            "amount of ZeroFi coin tokens to transfer {}, lender credit key {}",
            (withdraw_amount as f64 / ZeroFi_COIN_DECIMALS as f64),
            lender_ZeroFi_coin_account_to_credit.key.to_string()
        );
        invoke_signed(
            &transfer_withdraw_amount_to_lender_ix,
            &[
                ZeroFi_coin_vault_account.clone(),
                lender_ZeroFi_coin_account_to_credit.clone(),
                pda_account.clone(),
                token_program.clone(),
            ],
            &[&[&b"ZeroFiFinance"[..], &[bump_seed]]],
        )?;
        ...
        let vault_balance_decreased = ZeroFi_coin_vault_balance_before
            .checked_sub(ZeroFi_coin_vault_balance_after)
            .unwrap();
    ...
    fn process_withdraw_collected_loan_funds(
        ...
        let borrower_ZeroFi_ata_to_credit = next_account_info(account_info_iter)?;

        let ZeroFi_coin_vault_account = next_account_info(account_info_iter)?;

        /*
        let ZeroFi_vault_pubkey = utils::get_ZeroFi_vault_pubkey();

        if ZeroFi_vault_pubkey != *ZeroFi_coin_vault_account.key {
            return Err(ZeroFiError::ZeroFiVaultAccountDoesNotMatched.into());
        }
        ...
        */
        ...
        if loan_info_header_data.total_amount_lended < loan_info_header_data.total_loan_amount {
            return Err(ZeroFiError::CollectedLoanFundsAlreadyWithdrawn.into());
        }

        if loan_info_header_data.borrower_main_acc_pubkey != *borrower_main_account.key {
            return Err(ZeroFiError::BorrowerAccountMismatched.into());
        }
        ...
        let transfer_collected_loan_funds_to_borrower_ix = spl_token::instruction::transfer(
            token_program.key,
            ZeroFi_coin_vault_account.key,
            borrower_ZeroFi_ata_to_credit.key,
            &pda,
            &[&pda],
        ...
        msg!(
            "amount of ZeroFi coin tokens to transfer {}, lender credit key {}",
            (loan_info_header_data.total_amount_lended as f64 / ZeroFi_COIN_DECIMALS as f64),
            borrower_ZeroFi_ata_to_credit.key.to_string()
        );
        invoke_signed(
            &transfer_collected_loan_funds_to_borrower_ix,
            &[
                ZeroFi_coin_vault_account.clone(),
                borrower_ZeroFi_ata_to_credit.clone(),
                pda_account.clone(),
                token_program.clone(),
            ],
            &[&[&b"ZeroFiFinance"[..], &[bump_seed]]],
    ...
    fn process_transfer_ZeroFi_vault_account_ownership(
        accounts: &[AccountInfo],
        program_id: &Pubkey,
    ) -> ProgramResult {
        let account_info_iter = &mut accounts.iter();
        ...
        let ZeroFi_vault_account = next_account_info(account_info_iter)?;

        let (pda, _nonce) = Pubkey::find_program_address(&[b"ZeroFiFinance"], program_id);

        let rent = Rent::get()?;

        if !rent.is_exempt(
            ZeroFi_vault_account.lamports(),
            ZeroFi_vault_account.data_len(),
        ) {
            return Err(ZeroFiError::NotRentExempt.into());
        }
        let token_program = next_account_info(account_info_iter)?;
        ...
        let ZeroFi_vault_owner_change_ix = spl_token::instruction::set_authority(
            token_program.key,
            ZeroFi_vault_account.key,
            Some(&pda),
        ...
        msg!("Calling the token program to transfer ZeroFi vault account ownership to program...");
        invoke(
            &ZeroFi_vault_owner_change_ix,
            &[
                ZeroFi_vault_account.clone(),
    ...
    fn process_initialize_lenders_storage_account(accounts: &[AccountInfo]) -> ProgramResult {
        ...
        if lenders_storage_data_byte_array[0] != 0 {
            return Err(ZeroFiError::LendersStorageDataAlreadyInitialized.into());
        }
    ...
    fn process_initialize_borrower_storage_account(
        ...
        */
        ...
        if borrower_data.is_initialized() {
            return Err(ZeroFiError::BorrowerAccountAlreadyInitialized.into());
        }
    ...
    fn process_initialize_guarantor_storage_account(
        ...
        */
        ...
        if guarantor_data.is_initialized() {
            return Err(ZeroFiError::GuarantorAccountAlreadyInitialized.into());
        }
    ...
    //On each airdrop, users will get 250 ZeroFiCoin test tokens. A user can airdrop a maximum of 10 times.
    fn process_airdrop_users_with_ZeroFi_test_coins(
        accounts: &[AccountInfo],
        program_id: &Pubkey,
    ) -> ProgramResult {
        ...
        let ZeroFi_coin_airdrop_user_storage_account = next_account_info(account_info_iter)?;

        let rent = Rent::get()?;

        if !rent.is_exempt(
            ZeroFi_coin_airdrop_user_storage_account.lamports(),
            ZeroFi_coin_airdrop_user_storage_account.data_len(),
        ) {
            return Err(ZeroFiError::NotRentExempt.into());
        }

        let expected_ZeroFi_coin_airdrop_user_storage_pubkey = Pubkey::create_with_seed(
            airdrop_user_main_account.key,
            "ZeroFiFinanceAirdrop",
            program_id,
        )?;

        if expected_ZeroFi_coin_airdrop_user_storage_pubkey
            != *ZeroFi_coin_airdrop_user_storage_account.key
        {
            return Err(ZeroFiError::AccountMismatched.into());
        }

        let mut ZeroFi_coin_airdrop_user_storage_byte_array =
            ZeroFi_coin_airdrop_user_storage_account.try_borrow_mut_data()?;
        let stored_amount = u64::from_le_bytes(
            ZeroFi_coin_airdrop_user_storage_byte_array[0..8]
                .try_into()
                .unwrap(),
        );
        if stored_amount >= max_amount_to_airdrop {
            return Err(ZeroFiError::UserAlreadyAirdroped.into());
        }
        let new_total_airdrop_amount_for_user: u64 =
            stored_amount.checked_add(amount_to_airdrop).unwrap();
        ZeroFi_coin_airdrop_user_storage_byte_array
            .copy_from_slice(&new_total_airdrop_amount_for_user.to_le_bytes());

        let user_ZeroFi_coin_associated_token_to_credit_account =
            next_account_info(account_info_iter)?;

        let airdrop_vault_ZeroFi_coin_account = next_account_info(account_info_iter)?;
        ...
        // we can also store bump_seed to save computations
        let (pda, bump_seed) = Pubkey::find_program_address(&[b"ZeroFiFinanceAirdrop"], program_id);
        if pda != *pda_account.key {
            return Err(ZeroFiError::PdaAccountDoesNotMatched.into());
        }

        let airdrop_vault_account_data =
            TokenAccount::unpack(&airdrop_vault_ZeroFi_coin_account.data.borrow())?;

        let airdrop_vault_balance_before = airdrop_vault_account_data.amount;

        let transfer_ZeroFi_coin_to_airdroper_ix = spl_token::instruction::transfer(
            token_program.key,
            airdrop_vault_ZeroFi_coin_account.key,
            user_ZeroFi_coin_associated_token_to_credit_account.key,
            &pda,
            &[&pda],
            amount_to_airdrop,
        )?;
        ...
        invoke_signed(
            &transfer_ZeroFi_coin_to_airdroper_ix,
            &[
                airdrop_vault_ZeroFi_coin_account.clone(),
                user_ZeroFi_coin_associated_token_to_credit_account.clone(),
                pda_account.clone(),
                token_program.clone(),
            ],
            &[&[&b"ZeroFiFinanceAirdrop"[..], &[bump_seed]]],
        )?;
    ...
    fn process_transfer_airdrop_vault_account_ownership(
        ...
        let (pda, _nonce) = Pubkey::find_program_address(&[b"ZeroFiFinanceAirdrop"], program_id);

        let rent = Rent::get()?;

        if !rent.is_exempt(
            airdrop_vault_ZeroFi_coin_account.lamports(),
            airdrop_vault_ZeroFi_coin_account.data_len(),
        ) {
            return Err(ZeroFiError::NotRentExempt.into());
        }
        let token_program = next_account_info(account_info_iter)?;
        ...
        msg!("Calling the token program to transfer ZeroFi airdrop vault account ownership to program...");
        invoke(
